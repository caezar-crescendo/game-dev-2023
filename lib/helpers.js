...
export const updateUser = async(entry_id, nextPlayer = false) => {
    return await clientCMA
        .then((environment) => environment.getEntry(entry_id))
        .then((entry) => {
            if (!nextPlayer) { // score +1 point
                entry.fields.points['en-US'] = entry.fields.points['en-US'] + 1;
                entry.fields.playerTurn['en-US'] = false;
            } else {
                entry.fields = {
                    ...entry.fields,
                    playerTurn: {
...
export const updateGameSettings = async(entry_id, data) => {
    return await clientCMA
        .then((environment) => environment.getEntry(entry_id))
        .then((entry) => {
            entry.fields.blocksArangement['en-US'] = data;
            return entry.update();
        })
        .then(async(entry) => {
            console.log(`Entry ${entry.sys.id} updated.`);
